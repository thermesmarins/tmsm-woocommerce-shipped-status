 ...
 * @subpackage Tmsm_Woocommerce_Shipped_Status/admin
 */
 ...
 */
class Tmsm_Woocommerce_Shipped_Status_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/tmsm-woocommerce-shipped-status-admin.css', array('woocommerce_admin_styles'), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/tmsm-woocommerce-shipped-status-admin.js', array( 'jquery' ), $this->version, false );
	...
	function rename_views_filters($views){
		foreach($views as &$view){
			//$view = str_replace(_x( 'Processing', 'Order status', 'woocommerce' ), _x( 'Paid', 'Order status', 'tmsm-woocommerce-shipped-status' ), $view);
			$view = str_replace(_x( 'Completed', 'Order status', 'woocommerce' ), _x( 'Shipped', 'Order status', 'tmsm-woocommerce-shipped-status' ), $view);
			$view = str_replace('Processed', _x( 'Processed', 'Order status', 'tmsm-woocommerce-shipped-status' ), $view);
		}
		return $views;
	}
	...
	function admin_order_preview_actions($actions, $order){
		...
		$status_actions['complete']['name'] =  _x( 'Shipped', 'Order status', 'tmsm-woocommerce-shipped-status' );
		...
		if ( $order->has_status( array( 'processing', 'completed' , 'complete' ) ) ) {
			$status_actions['processed'] = array(
				...
				'name'   => _x( 'Processed', 'Order status', 'tmsm-woocommerce-shipped-status' ),
				'action' => 'processed',
			);
		}
	...
	function rename_bulk_actions(array $actions){
		//$actions['mark_processing'] = __( 'Mark paid', 'tmsm-woocommerce-shipped-status' );

		$actions['mark_completed']  = __( 'Mark shipped', 'tmsm-woocommerce-shipped-status' );
		$actions['mark_processed'] = __('Mark as processed', 'tmsm-woocommerce-shipped-status');

		return $actions;
	}

	/**
	 * Rename order statuses: completed > shipped
	 *
	 * @param $statuses
	 *
	 * @return array
	...
	function rename_order_statuses($statuses){
		...
		$statuses['wc-completed'] = _x( 'Shipped', 'Order status', 'tmsm-woocommerce-shipped-status' );
		$statuses['wc-processed'] = _x( 'Processed', 'Order status', 'tmsm-woocommerce-shipped-status' );
	...
	function admin_order_actions_processed($actions, $order){
		...
		$actions['complete']['name'] = _x( 'Ship', 'Change order status', 'tmsm-woocommerce-shipped-status' );
		...
		if ( $order->has_status( array( 'processing', 'completed' ) ) ) {
			...
			$actions['processed'] = array(
				...
				'name'   => __( 'Mark as processed', 'tmsm-woocommerce-shipped-status' ),
	...
	function admin_action_mark_processed() {
		...
		foreach( $_REQUEST['post'] as $order_id ) {
			...
			$order_note = __('Status changed to Processed', 'tmsm-woocommerce-shipped-status');
